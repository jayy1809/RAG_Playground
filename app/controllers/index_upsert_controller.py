...
class IndexUpsertController:
    def __init__(
            self, 
            index_upsert_usecase = Depends(IndexUpsertUseCase)
        ):
    ...
    async def index_upsert(self, request):
        ...
        already_upserted = await self.index_upsert_repository.find_matching_index_upsert(dimension, similarity_metric, file_name, embed_model)
        ...
        return await self.index_upsert_usecase.index_upsert(request, already_index)
