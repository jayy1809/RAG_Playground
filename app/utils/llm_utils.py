...
class LLMUtils:
    ...
    async def generate_questions(self, chunk: str):
        llm = ChatGroq(
            model="mixtral-8x7b-32768", api_key=self.settings.GROQ_API_KEY
        )
    ...
    async def generate_multi_chunk_question(self, chunks: List[Dict]):
        ...
        try:
            ...
            return {
                ...
                "relevant_ids": [
                    chunk["_id"] for chunk in chunks[:2]
                ],  # Default to first two chunks
            }
            result = json.loads(response.content.strip())
            ...
            raise ValueError(
                f"Invalid multi-chunk response from LLM: {response.content} - Error: {str(e)}"
            )
