...
class EvaluationService:
    ...
    def _discounted_cumulative_gain_at_k(
        retrieved_docs: List[Dict], ground_truth: List[int], k: int
    ) -> float:
        """
    ...
    def normalized_discounted_cumulative_gain_at_k(
        retrieved_docs: List[Dict], ground_truth: List[Dict], k: int
    ) -> float:
        """
        ...
        try:
            ...
            for i in range(k):
                dcg_at_i = EvaluationService._discounted_cumulative_gain_at_k(
                    retrieved_docs, ground_truth_texts_ids, i + 1
                )
                idcg_at_i = (
                    EvaluationService._ideal_discounted_cumulative_gain_at_k(
                        ground_truth_texts_ids, i + 1
                    )
                )
    ...
    @staticmethod
    def precision_at_k(
        retrieved_docs: List[Dict], ground_truth: List[Dict], max_k: int = None
    ) -> Dict[str, float]:
        """
    ...
    def f1_score_at_k(
        ...
        try:
            ...
            for k in range(1, max_k + 1):
                ...
                # Calculate recall
                recall = (
                    matches / float(len(ground_truth_ids))
                    if ground_truth_ids
                    else 0.0
                )
    ...
    @staticmethod
    def reciprocal_rank(
        retrieved_docs: List[Dict], ground_truth: List[Dict]
    ) -> float:
        """
