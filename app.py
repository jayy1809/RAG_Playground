...
def hybrid_search_pipeline(key: str):
    ...
    if dense_embedding_model and dense_dimension:
        ...
        region = st.selectbox("Enter Region:", regions, key="region" + key)

        if index_name and cloud_provider and region:

            create_index_button = st.button(
                "Create Index and Upsert Dataset", key="create_index" + key
            )
            if create_index_button:
                st.write(
                    f"Index '{index_name}' created and dataset upserted successfully!"
                )
            ...
            if st.session_state.index_button_1_clicked:
                ...
                with col1:
                    similarity_search_button = st.button(
                ...
                with col2:
                    reset_results_button = st.button(
                        "Reset Results",
    ...
    if uploaded_file:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                "http://127.0.0.1:8000/upload-files", json=payload, timeout=75.0
            )
